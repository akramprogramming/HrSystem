...
try {
    if ($auth->isAdmin()) {
        ...
        ");
        $moderators = $stMods->fetchAll();
    } else {
        $selectedModeratorId = (string)$currentUserId;
    }
} catch (Throwable $e) {
    $errors[] = 'تعذر تحميل قائمة المشرفين: ' . $e->getMessage();
}
...
try {
    if ($auth->isAdmin()) {
        ...
        ");
        $stUsers->execute([':mid' => $currentUserId]);
        $users = $stUsers->fetchAll();
    }
} catch (Throwable $e) {
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($auth->isAdmin()) {
        $selectedModeratorId = (string)($_POST['moderator_id'] ?? '');
    } else {
        $selectedModeratorId = (string)$currentUserId;
    }
    ...
    if ($selectedModeratorId === '' || !ctype_digit($selectedModeratorId) || (int)$selectedModeratorId <= 0) {
        $errors[] = 'يجب اختيار مشرف صحيح.';
    }
        ...
        try {
            if ($auth->isAdmin()) {
                $chkMod = $db->prepare("
                    ...
                    LIMIT 1
                ");
                $chkMod->execute([':id' => (int)$selectedModeratorId]);
                if (!$chkMod->fetch()) {
                    $errors[] = 'المشرف المحدد غير صالح.';
                }
            } else {
                if ((int)$selectedModeratorId !== $currentUserId) {
                    $errors[] = 'غير مصرح بتغيير المشرف.';
                }
            }
        } catch (Throwable $e) {
            $errors[] = 'تعذر التحقق من المشرف: ' . $e->getMessage();
        }
    }

    // تحقق من الموظف + تبعيته للمشرف المختار
    if (empty($errors)) {
        try {
            $chkEmp = $db->prepare("
                SELECT u.id
            ...
            $chkEmp->execute([
                ':uid' => (int)$assignedTo,
                ':mid' => (int)$selectedModeratorId,
            ]);

            if (!$chkEmp->fetch()) {
                $errors[] = 'الموظف غير مرتبط بالمشرف المختار.';
            }
        } catch (Throwable $e) {
                ...
                if (!isset($allowed[$mime])) {
                    ...
                    if (!is_dir($uploadDirFs) && !@mkdir($uploadDirFs, 0775, true) && !is_dir($uploadDirFs)) {
                        ...
                        if (!move_uploaded_file($tmp, $destFs)) {
                            ...
                            $imagePath = '/public/uploads/tasks/' . $filename;
                        }
                    }
                }
            }
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt->execute([
                ...
                ':moderator_id' => (int)$selectedModeratorId,
            ...
            $startDate = date('Y-m-d');
            $dueDate = '';
            $selectedModeratorId = $auth->isModerator() ? (string)$currentUserId : '';
        } catch (Throwable $e) {
...
$pageTitle = $auth->isAdmin() ? 'إضافة مهمة جديدة (مدير)' : 'إضافة مهمة جديدة (مشرف)';
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .card { background:#fff; border-radius:14px; padding:20px; box-shadow:0 8px 22px rgba(0,0,0,.08); }
        ...
        .btn { border:0; border-radius:10px; padding:10px 14px; color:#fff; text-decoration:none; cursor:pointer; font-size:15px; background:#1976d2; }
        ...
        <form method="post" action="" enctype="multipart/form-data">
            ...
            <div class="grid">
                <div class="full">
                    ...
                    <input type="text" id="title" name="title" maxlength="255" value="<?php echo htmlspecialchars($title, ENT_QUOTES, 'UTF-8'); ?>" required>
                </div>
                ...
                <?php if ($auth->isAdmin()): ?>
                <div>
                    <label for="moderator_id">المشرف المسؤول *</label>
                    <select id="moderator_id" name="moderator_id" required>
                        <option value="">-- اختر مشرف --</option>
                        <?php foreach ($moderators as $m): $mid=(int)$m['id']; ?>
                            <option value="<?php echo $mid; ?>" <?php echo ((string)$mid === $selectedModeratorId) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars((string)$m['full_name'], ENT_QUOTES, 'UTF-8'); ?>
                                (<?php echo htmlspecialchars((string)$m['username'], ENT_QUOTES, 'UTF-8'); ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div class="help">اختر المشرف المسؤول عن متابعة هذه المهمة.</div>
                </div>
                <?php else: ?>
                    <input type="hidden" name="moderator_id" value="<?php echo (int)$currentUserId; ?>">
                <?php endif; ?>
                    ...
                    <select id="assigned_to" name="assigned_to" required>
                        ...
                        <?php foreach ($users as $u): $uid=(int)$u['id']; ?>
                            <option value="<?php echo $uid; ?>" <?php echo ((string)$uid === $assignedTo) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars((string)$u['full_name'], ENT_QUOTES, 'UTF-8'); ?> (<?php echo htmlspecialchars((string)$u['username'], ENT_QUOTES, 'UTF-8'); ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" value="<?php echo htmlspecialchars($startDate, ENT_QUOTES, 'UTF-8'); ?>" required>
                </div>

                <div>
                    <label for="due_date">تاريخ التسليم</label>
                    <input type="date" id="due_date" name="due_date" value="<?php echo htmlspecialchars($dueDate, ENT_QUOTES, 'UTF-8'); ?>">
                </div>
            </div>

            <div class="actions">
                <button class="btn" type="submit">حفظ المهمة</button>
                <a class="btn btn-gray" href="/admin/tasks_list.php">عرض المهام</a>
                <a class="btn btn-gray" href="<?php echo $auth->isAdmin() ? '/admin/dashboard.php' : '/moderator/dashboard.php'; ?>">العودة للوحة التحكم</a>
            </div>
        </form>
    </div>
</div>
